######################################################
# -*- coding: utf-8 -*-
# File Name: end2end_fused_mxnet.py
# Author: Qian Li
# Created Date: 2017-12-02
# Description: Test fused decoder+mxnet Lambdas!
######################################################
...
DOWNLOAD_PREFIX = 'fused-decode-mxnet-output'
...
def invoke_decoder_lambda(bucketName, uploadPrefix, num_rows, batchSize):
  ...
  def invoke_lambda(bucketName, filePrefix, startFrame, batchSize):
    ...
    try:
      ...
      response = client.invoke(FunctionName='fused-decode-mxnet',
...
def start_fused_mxnet_pipeline(test_video_path='videos/example.mp4', 
...
if __name__ == '__main__':
  ...
  start = now()
  start_fused_mxnet_pipeline(test_video_path, out_dir, batch, load_to_disk)
  stop = now()
  delta = stop - start
