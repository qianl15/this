...
with Database() as db:
  ...
  [input_table], failed = db.ingest_videos([
      ('test_raw', example_video_path)], force=True)
  ...
  job = Job(
      op_args={
          frame: db.table('test_raw').column('frame'),
          output_op: 'test_hist'
      }
  )
  ...
  # the top left.
  output_table.profiler().write_trace('test_hist.trace')
  ...
  for (frame_index, frame_hists) in video_hists:
      ...
      num_rows += 1
  assert num_rows == db.table('test_raw').num_rows()
